...
export default function ExpandableMenuNavbar({
  ...
  const fallbackOpenHeight = 360 - WRAPPER_PADDING;
  const openHeight =
  ...
  return (
    <div
      ref={containerRef}
      className="pointer-events-none fixed inset-x-0 top-2 z-50 h-90 max-h-[calc(100dvh-1rem)] w-full"
    >
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="pointer-events-auto fixed inset-0 z-0 bg-foreground/5 backdrop-blur-xs"
        ...
        >
          ...
          >
            ...
            <span className="flex items-center gap-2 text-xs text-muted-foreground">
              <ComboKbd className="hidden sm:inline-flex" size="sm" keys={[shortcut]} />
          ...
          <AnimatePresence initial={false}>
            {open && (
              <motion.div
                ...
                className="min-h-0 flex-1 touch-pan-y overflow-y-auto overscroll-contain border-t border-border [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
              >
